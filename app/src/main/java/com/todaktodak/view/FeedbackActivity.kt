...
class FeedbackActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            // if tts initialization succeeded
            ...
            if (languageStatus == TextToSpeech.LANG_MISSING_DATA || languageStatus == TextToSpeech.LANG_NOT_SUPPORTED) {
                ...
                val speechStatus: Int = tts.speak(data, TextToSpeech.QUEUE_FLUSH, null, null)
                if (speechStatus == TextToSpeech.ERROR) {
                    Toast.makeText(this, "음성 지원 불가", Toast.LENGTH_LONG).show()
                }
            }
        } else {
            // If tts initialization failed
            Toast.makeText(this, "음성 전환 오류", Toast.LENGTH_LONG).show()
        }
    }
    // Shutdown the tts engine when 'Pause'
        ...
        call.enqueue(object : Callback<String> {
            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (response.isSuccessful) {
                    Log.d("GetRESPONSE: ", "SUCCESS"+response.body().toString())
    ...
    private var activityResult: ActivityResultLauncher<Intent> = registerForActivityResult(
        ...
        if (result.resultCode == TextToSpeech.Engine.CHECK_VOICE_DATA_PASS) {
            ...
            startActivities(arrayOf(installIntent))
        }
    }
}
//
